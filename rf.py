...
Original file is located at
    https://colab.research.google.com/drive/10k-bOBIvcn6XaQ0EbDwi5hMJP9B7QjGg
"""
...
dfx = df[df.columns[3:11]]
print(dfx.drop_duplicates())
#classes
Y1 = df["type"].to_numpy()

print("\nAtaques representam {:.4f}% do dataset.\n".format((df[df.type == 1].shape[0] / df.shape[0]) * 100))
fig, eixos = plt.subplots()
pizza = eixos.pie([df[df.type == 1].shape[0],df[df.type == 0].shape[0]], labels=['attack','normal'],
                    autopct='%1.1f%%', colors=['gold', 'lightskyblue'])
# Define o título deste gráfico
eixos.set_title('Distribuição dataset')
...
for i, (train, test) in enumerate(cv.split(X1, Y1)):
    rf.fit(X1[train], Y1[train])
    grid_cv = GridSearchCV(rf, param_grid_rf, scoring="roc_auc", n_jobs=-1, cv=3).fit(X1, Y1)
    print("Param for GS", grid_cv.best_params_)
    print("CV score for GS", grid_cv.best_score_)
    break

# Cria o classificador com os melhores parametros
...
for i, (train, test) in enumerate(cv.split(X1, Y1)):
    ...
    rf, X1[test], Y1[test], name="precision Recall Random Florest"
    )
    _ = display.ax_.set_title("2-class Precision-Recall curve")
    ConfusionMatrixDisplay.from_estimator(rf,X1[test], Y1[test])
