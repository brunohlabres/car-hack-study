...
Original file is located at
    https://colab.research.google.com/drive/1DtY-lP8z-uUevNLhQy4Cg7bVcpzS-IDD
"""
...
for i, (train, test) in enumerate(cv.split(X1, Y1)):
    ...
    grid_cv = GridSearchCV(mlp, param_grid_mlp, scoring="roc_auc", n_jobs=-1, cv=5).fit(X1, Y1)
...
for i, (train, test) in enumerate(cv.split(X1, Y1)):
    ...
    mlp, X1[test], Y1[test], name="precision Recall MLP"
    )
    _ = display.ax_.set_title("2-class Precision-Recall curve MLP")
    ConfusionMatrixDisplay.from_estimator(mlp,X1[test], Y1[test])
